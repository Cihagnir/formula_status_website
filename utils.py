...
class Utils:  # ==== Configuration Utils ====
    @staticmethod
    def general_config_reader():
        general_config_var: Types.general_config = Types.general_config()
        config_parser = configparser.ConfigParser()
        _ = config_parser.read(CONFIG_FILE_PATH)

        general_config_var.global_debug = config_parser.getboolean(
            "General", "global_debug"
        )
        general_config_var.database_setting = config_parser.get(
            "General", "database_setting"
        )

        del config_parser
        return general_config_var

    @staticmethod
    def database_config_reader(database_setting: str | None):
        database_config_var: Types.database_config = Types.database_config()
        config_parser = configparser.ConfigParser()
        _ = config_parser.read(CONFIG_FILE_PATH)
        ...
        if database_setting is not None:
            if "local" in database_setting:
                if LOCAL_DEBUG:
                    print("UTILS ::: Local database connection config hold")

                database_config_var.user = config_parser.get("Database", "local_user")
                database_config_var.host = config_parser.get("Database", "local_host")
                database_config_var.port = config_parser.getint(
                    "Database", "local_port"
                )
                database_config_var.password = config_parser.get(
                    "Database", "local_password"
                )
                Types.database_config.database = config_parser.get(
                    "Database", "local_database"
                )

            elif "AWS" in database_setting:
                if LOCAL_DEBUG:
                    print("UTILS ::: AWS database connection config hold")
                ...
                Types.database_config.port = config_parser.getint(
                    "Database", "aws_port"
                )
                Types.database_config.password = config_parser.get(
                    "Database", "aws_password"
                )
                Types.database_config.database = config_parser.get(
        ...
        return database_config_var

    # ==== Descriptive Function Utils ====

    @staticmethod
    def quantile_to_bound(
        lower_quant: float,
    ...
    def position_matcher(first_lap: DataFrame, second_lap: DataFrame):
        ...
        search_window = max(2, (len(lap_two) - len(lap_one)))
        ...
        for index in range(len(lap_one_cords)):
            ...
            lap_one_mat = stack(
                [lap_one_cords[index, :] for _ in range(sub_lap_two.shape[0])]
            )
